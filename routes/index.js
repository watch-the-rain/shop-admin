  ...
  conn.query(sql_str, (error, results, fields) => {
    if (error) {
      ...
      if (results[0]) {
        ...
        // 商品不存在
        let add_sql =
          "INSERT INTO cart(goods_id, goods_name, thumb_url, price, buy_count, is_pay, user_id, counts) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        let sql_params = [
          goods_id,
          goods_name,
          thumb_url,
...
router.post("/api/order/create", (req, res) => {
  ...
  let sqlStr =
    "INSERT INTO tb_order(order_num,user_id,goods_id,order_status) VALUES (?,?,?,?)"
 ...
 * 订单列表-用户订单
 */
router.get("/api/order/list", (req, res) => {
  // 获取数据
  const user_id = req.query.user_id
  let sqlStr = `SELECT * FROM tb_order,recommend WHERE tb_order.goods_id=recommend.goods_id `
  if (user_id) {
 ...
 * 订单列表-用户订单
 */
router.post("/api/order/update_status", (req, res) => {
  // 获取数据
  const user_id = req.query.user_id
  const status = req.body.status
  const id = req.body.id
  let sqlStr = `UPDATE tb_order SET order_status = ${status} where id = ${id}`
  conn.query(sqlStr, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ err_code: 0, message: "服务器内部错误!" })
